...
All topics in consumer `spring.cloud.stream.bindings.<bindingName>.destination` and `spring.cloud.stream.solace.bindings.<bindingName>.consumer.queueAdditionalSubscriptions`
can contain the [standard (* and >) solace wildcards](https://docs.solace.com/PubSub-Basics/Wildcard-Charaters-Topic-Subs.htm).
It is also possible to use wildcards like `sensor/temperature/<station>/>`. In this case `<station>` will be translated to `*` for the broker.
The inbound messages will have a header `destinationVariables` what is a Map<String, String> containing the key `station` the value of this part of the destination.
